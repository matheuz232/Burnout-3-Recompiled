...
**Goal:** Add an injectable Windows-side R5900 host syscall boundary, integrate it into `R5900BlockDispatcher`, preserve current trap behavior when no service is configured, and prove deterministic handled/unsupported/fault behavior without putting syscall logic into generated x64.

**Architecture:** Add a narrow `IR5900HostSyscallService` interface plus a production `R5900HostSyscallService` shell beside the Windows dispatcher. The dispatcher recognizes decoded `SYSCALL` as a host boundary, executes any native prefix first, calls the injected service, and resumes at `guest_pc + 4` only after `Handled`; unsupported and fault outcomes stop at the exact syscall PC. This plan deliberately ends before named `SetupThread` semantics; that is a separate milestone because its kernel-visible stack/result behavior requires its own evidence and review.
...
- Windows x86-64 only for host-syscall integration; portable targets must continue to configure/build as before.
...
- A null service pointer preserves current `Trap` behavior and existing callers remain source-compatible.
- `blocks_executed` and `instructions_executed` retain current native-dispatch meanings; handled syscalls are counted only in `syscalls_handled`.
- No Burnout 3 ELF bytes, ISO data, assets, dumps, or proprietary payloads are committed.
- Do not implement `SetupThread`, `SetupHeap`, scheduling, or any other named EE kernel service in this plan.
- Public ABI evidence reserved for the next milestone: `SetupThread` selector `0x3c`/60 is in GPR3 (`v1`), arguments are in GPR4..GPR8, and startup consumes GPR2 (`v0`) as the returned stack pointer. This infrastructure may report the selector but must not claim `SetupThread` is implemented.
...
- `CMakeLists.txt` — compile the service into `b3r_recompiler_dispatcher_x64` and register two Windows tests.
- `README.md` — document infrastructure without claiming a named kernel handler.
- `docs/PROGRESS.md` — record final CI evidence and the next `SetupThread` gate.

No changes are required in `r5900_ir.*`, `r5900_ir_executor.*`, `r5900_x64_backend.*`, or `Ps2MemoryMap`.
...
- [ ] **Step 1: Add the failing build/test surface**

Change the existing dispatcher library to compile the new source:
...
Inside the Windows test block add:
...
int main() {
    ...
    const auto unsupported = service.handle(
        R5900HostSyscallRequest{0x00100000u, 0x0000000cu}, state, memory);
    ...
    const auto malformed = service.handle(
        R5900HostSyscallRequest{0x00100004u, 0x00000000u}, state, memory);
...
Expected: configure/build fails because `r5900_host_syscall_service.h/.cpp` do not exist.
...
- [ ] **Step 3: Create the public service header**
...
- [ ] **Step 4: Implement the minimum production shell**
...
std::uint32_t ee_syscall_selector(const R5900IrExecutionState& state) noexcept {
    return static_cast<std::uint32_t>(state.gpr[3].low64);
}
...
The production shell deliberately has no `Handled` selector.
...
### Task 2: Dispatcher service injection and host-boundary outcomes
...
- [ ] **Step 2: Create the complete failing dispatcher test fixture**

Create `tests/r5900_block_dispatcher_syscall_windows_tests.cpp` with this complete synthetic ELF fixture and fake service:
...
Expected: compile failure because the dispatcher contract does not yet expose `host_syscalls`, `syscalls_handled`, `UnsupportedSyscall`, or `HostSyscallFailure`.
...
In `src/recompiler/windows/r5900_block_dispatcher.h`, add:

```cpp
#include "recompiler/windows/r5900_host_syscall_service.h"
```

Add immediately after `Trap`:
...
The pointer is non-owning and must outlive `run()`.

- [ ] **Step 5: Add one dispatcher-side host-boundary helper**

Near existing local helpers in `r5900_block_dispatcher.cpp`, add:
...
- [ ] **Step 6: Detect `SYSCALL` separately from generic system traps**

Before the block scan, add:

```cpp
const analysis::R5900InstructionSite* syscall_site = nullptr;
```

Replace the current generic system-class branch with:
...
Do not push `SYSCALL` into `body_sites` or `guest_words`.

- [ ] **Step 7: Handle an entry syscall before the empty-body return**

Before the existing `body_sites.empty()` early return, add:

```cpp
if (body_sites.empty() && !has_supported_transfer && syscall_site != nullptr) {
    const auto disposition = handle_host_syscall_boundary(
        options_.host_syscalls, memory_, *syscall_site, state, result, current_pc);
...
Leave the existing empty-body behavior unchanged for non-syscall boundaries.

- [ ] **Step 8: Handle a syscall after a successful native prefix**

After native execution has committed `blocks_executed`, `instructions_executed`, `current_pc`, and `next_pc`, but before the existing `boundary_reason` return, add:
...
This ordering is required: native prefix effects must be visible to the host service.
...
**Interfaces:**
- Consumes: host-boundary dispatcher integration from Task 2.
- Produces: startup-shaped regression evidence that BSS and OR native effects commit before the service call and a handled syscall resumes to the following instruction.

- [ ] **Step 1: Extend the synthetic executable by one deliberate post-syscall boundary**

In `make_memory()`, append after the existing `0x0000000cu` word:

```cpp
i_type(0x0eu, 1u, 1u, 1u), // XORI r1,r1,1: deliberate unsupported boundary
```
...
The existing null-service run must still stop at `kSyscallPc` with `Trap`, 10 native blocks, 28 selected native guest words, 3 cache misses, 7 cache hits, and 7 fast cache hits.

- [ ] **Step 2: Add the exact handled fake service**
...
- [ ] **Step 3: Add the handled continuation regression**

After the existing null-service assertions, run a fresh memory/state with the fake service:
...
- [ ] **Step 4: Run the integration regression**
...
Expected: PASS. The null-service case still traps at the syscall; the injected handled case resumes to `kPostSyscallPc`, preserves native counters, records one handled syscall, and stops on XORI.
...
### Task 4: Full regression, CI evidence, documentation, and handoff
...
**Interfaces:**
- Consumes: Tasks 1-3.
- Produces: auditable milestone evidence and a clean boundary for a separate `SetupThread` plan.
...
Expected: every registered test passes. Record the actual total; do not predict a fixed count.

- [ ] **Step 3: Push the implementation head and verify the full Windows CI workflow**

Require these stages on the final implementation head:
...
Do not label the milestone `CI_VALIDATED` before that final-head workflow completes successfully.

- [ ] **Step 4: Update `README.md` with exact capability language**

Add these facts to the current milestone description:

```text
- an injectable Windows R5900 host-syscall boundary keeps EE kernel HLE outside generated x64;
- null-service SYSCALL remains a deterministic Trap;
...
- the synthetic BSS-clear -> register OR path crosses a handled syscall boundary and reaches the following unsupported instruction.
```

Also add:

```text
SetupThread itself is not implemented by this milestone. The production host service validates/diagnoses the EE syscall selector and returns Unsupported until a named handler is separately validated.
```

- [ ] **Step 5: Update `docs/PROGRESS.md` using actual evidence**

Before CI is green, use status `IN_PROGRESS`. After final-head CI is green, change it to `CI_VALIDATED` and record the actual commit SHA, workflow run ID, job ID, and CTest total.

The completed row must describe:

```markdown
| R5900 host syscall service v0 | CI_VALIDATED | Injectable `IR5900HostSyscallService` keeps syscall HLE outside generated x64. Null service preserves `Trap`; handled/unsupported/fault tests prove PC+4 resume, exact-PC stop behavior, separate `syscalls_handled` accounting, and BSS-clear -> OR -> SYSCALL continuation. Production service still reports unimplemented EE selectors as unsupported; `SetupThread` is the next milestone. |
```
...
- [ ] **Step 7: Verify the documentation commit itself**

The documentation commit changes the final head, so verify the Windows workflow for that exact head before claiming completion. If docs-only CI is not automatically triggered, run the repository's supported manual Windows workflow for that head and record its evidence.

---

## Scope Boundary for the Next Plan

After this infrastructure plan is complete, create a separate `SetupThread` implementation plan. It may rely on these established observations but must define and test the kernel-visible stack/result semantics before selector `0x3c` becomes `Handled` in production:

```text
selector: GPR3 / v1 = 60 (0x3c)
arguments: GPR4..GPR8 = gp, stack, stack_size, args, root_func
return consumed by startup: GPR2 / v0, immediately moved to sp
Burnout 3 boundary: SYSCALL @ 0x001001c8
next observed startup selector: SetupHeap 0x3d
```

The next plan must define the exact `SetupThread` stack/result calculation and any required kernel bookkeeping with testable evidence; it must not infer those semantics solely from the function name.

## Plan Self-Review

- **Spec coverage:** service interface, production shell, null-service compatibility, handled/unsupported/fault outcomes, PC+4 resume, separate syscall accounting, no syscall IR/x64 emission, BSS/OR integration, docs, and full Windows CI all have explicit tasks.
...
- **Placeholder scan:** every implementation/test step includes concrete code or an exact command; no `TBD`, `TODO`, “similar to”, or conditional implementation path remains.
- **Type consistency:** `IR5900HostSyscallService`, `R5900HostSyscallRequest`, `R5900HostSyscallResult`, `R5900HostSyscallStatus`, `host_syscalls`, `UnsupportedSyscall`, `HostSyscallFailure`, and `syscalls_handled` have one spelling/signature throughout.