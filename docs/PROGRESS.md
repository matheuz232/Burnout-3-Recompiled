...
| Static/binary recompiler | IN_PROGRESS | IR v0 plus a deterministic reference executor now exist; x86-64 backend/code generation and native guest execution remain TODO, and real ELF coverage should drive further opcode/backend scope |
...
No Burnout 3 game code is executed by this milestone. The reference executor is a deterministic semantic oracle for the current IR subset; there is still no x86-64 backend, native guest-code execution path, game initialization, graphics, audio, or input behavior. Hosted CI smoke evidence is not treated as physical-desktop performance certification.
