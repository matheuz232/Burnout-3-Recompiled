...
| R5900 IR v0 | CI_VALIDATED | Provenance-carrying non-executable IR lowers NOP, ADDU, ADDIU and ORI; integer writes explicitly use `Low64PreserveUpper64` for 128-bit EE GPRs, side-effect-free writes to GPR zero are discarded, and unsupported instructions emit no partial IR; Windows/MSVC suite passes 24/24 |
...
| Static/binary recompiler | IN_PROGRESS | IR v0 and the first explicit R5900 lowering path now exist; x86-64 backend/code generation and guest execution remain TODO, and real ELF coverage should drive further opcode/backend scope |
...
No game execution, simulation timing, graphics, audio, or input behavior is changed by `Burnout3PacingProbe` or the IR v0 work. The pacing probe measures only the existing 120 Hz Windows pacing path, while IR v0 is non-executable. Hosted CI smoke evidence is not treated as physical-desktop performance certification.
