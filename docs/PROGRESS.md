...
| 120 FPS Windows frame pacer | WORKING | Short hosted-CI telemetry remains healthy; `Burnout3PacingProbe` now provides a fixed-120-Hz 60-second-or-longer desktop capture path, but a normal physical Windows 10/11 capture is still required for the validation gate |
...
| `Burnout3Analyze` Windows artifact | CI_VALIDATED | `main`/manual Windows CI publishes `Burnout3Analyze-windows-x64`; latest verified pre-pacing-probe main artifact ID `9921137059` contains the analyzer executable plus usage guide |
| `Burnout3PacingProbe` CLI | CI_VALIDATED | Windows-only console tool keeps a fixed 120 Hz target; parser/app/help and a one-second 120-sample smoke pass in Windows/MSVC CI; default capture is 60 seconds / 7200 intervals and no performance PASS/FAIL is inferred |
| `Burnout3PacingProbe` Windows artifact | READY_FOR_MAIN_PUBLICATION | Feature-branch CI stages and validates `Burnout3PacingProbe.exe` plus `PACING-PROBE-USAGE.txt`; main/manual publication remains the final milestone gate before physical-desktop use |
...
`DIRECT_CALL_TARGETS` diagnostics used the same report-contract TDD pattern. RED run `33828724619` built successfully and passed 19/20 tests; only `r5900_analysis_report_tests` failed because target aggregation was absent. Renderer run `33828875766` proved grouping/filtering/order: `r5900_analysis_report_tests` passed, while the only remaining failure was the older byte-exact `ps2_elf_analysis_tests` expectation missing `DIRECT_CALL_TARGETS 0`. After updating only that expected text, GREEN run `33828970350` passed 20/20 plus frame-pacing telemetry and analyzer package staging/validation. PR #16 merge-ref run `33829331769` passed 20/20 on temporary merge SHA `9a24a115d13862a12ec7b67c7c635621695c6672`; merge commit `79b83039df6d58d3d997a1536021c32cadac6a33` then passed 20/20 in main run `33829442535` and published analyzer artifact ID `9921137059` (61,752-byte ZIP; SHA-256 `4b7c986a42c8ad0f18f73a16cb9ef697679156c236c2e1609fcd56b5f76e4d5d`). The synthetic fixture proves two static call sites grouped under target `0x00002000`, one under `0x00003000`, exclusion of an indirect call and a direct unresolved call, deterministic target ordering and byte-identical rendering when the graph call container is reversed.

`Burnout3PacingProbe` used four explicit TDD gates. Parser RED run `33832016872` failed exactly because the probe option header did not exist; parser GREEN run `33832153516` passed 21/21. App RED run `33832281693` failed exactly because the probe app header did not exist; app GREEN run `33832432978` passed 22/22, including a one-second 120-sample Windows capture. Executable RED run `33832644412` built successfully and passed 22/23, with only `burnout3_pacing_probe_help` not run because `Burnout3PacingProbe.exe` did not exist; executable GREEN run `33832757211` passed 23/23. Package RED run `33832885053` passed 23/23 plus a visible one-second probe smoke (`TARGET_HZ 120`, `FRAMES 120`, `SAMPLES 120`) and then failed only because `docs/PACING-PROBE-USAGE.txt` was absent. Package GREEN run `33832975764` passed 23/23, the visible probe smoke, existing frame-pacing telemetry, analyzer package staging/validation, and pacing-probe package staging/validation; both uploads were correctly skipped on the feature branch.

No R5900 decoder instruction support, reachability behavior, guest execution semantics, graphics, audio, input, or game initialization behavior was added by the pacing-probe milestone. The production `WindowsFramePacer` implementation was not changed. Remaining workflow warnings are external action/runtime deprecations.
...
2. run the CI-published `Burnout3PacingProbe-windows-x64` on a normal physical Windows 10/11 desktop for 60 seconds or longer and review the generated timing report; the one-second/240-frame hosted-CI evidence validates execution only, not physical-desktop pacing;
