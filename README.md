...
`Burnout 3 Recompiled - Test Build 0.1` bootstrap, static-analysis, and native R5900 startup-execution infrastructure through the real startup BSS-clear loop boundary, with `SQ` guest-memory writes, ordinary `BEQ`/`BNE`, branch-likely `BEQL`/`BNEL`, direct `J`/`JAL`, indirect `JR`/`JALR`, transfer-block fast cache replay, and an injectable host-syscall boundary kept outside generated x64.
...
- a production `R5900HostSyscallService` shell that validates the raw `SYSCALL`, extracts the EE selector from `GPR3/v1`, and deterministically reports unimplemented selectors as unsupported; no named EE kernel handler is enabled yet;
...
- a dedicated BSS-clear regression in which the native loop and post-loop register `OR` execute first, then an injected handled syscall crosses the host boundary without becoming IR/x64/cache content and reaches the deliberate unsupported instruction at `SYSCALL PC+4`, while the null-service path still traps at the original syscall;
...
`R5900HostSyscallService` is deliberately outside the generated-code path. The dispatcher detects a decoded `SYSCALL`, commits any supported native prefix first, and invokes the configured host service with exact guest-PC/raw-instruction provenance plus current R5900 state and `Ps2MemoryMap`. The production shell currently returns `Unsupported` for every valid selector. **`SetupThread` itself is not implemented by this milestone.** The next syscall milestone is to validate and implement its guest-visible stack/result semantics before selector `0x3c` may return `Handled` in production.
...
The game still does **not** boot. The real BSS-clear loop is represented by a dedicated native-dispatcher fixture and transfer-block fast cache replay, and the dispatcher now has a tested host-syscall boundary capable of resuming after an injected handled call. The production service still does not implement `SetupThread` or any other EE kernel service. Broader guest-memory loads/stores, additional control flow including `BLEZL`/`BGTZL` and REGIMM likely/link-likely variants, named syscall/HLE handlers, graphics, audio, input, menus, and gameplay remain unimplemented. The host-syscall infrastructure is CI-validated on synthetic data; the legally supplied external ELF has not been run through this expanded Windows native path in this environment.
